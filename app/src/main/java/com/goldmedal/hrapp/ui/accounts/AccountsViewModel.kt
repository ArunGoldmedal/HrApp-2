...
class AccountsViewModel @Inject constructor(
    ...
    var strPartyCin: String? = null
    var strSearchText: String? = null
    var updateAmount: String? = null
        ...
        Coroutines.main {
            try {
                ...
                getIncreaseLimitResponse[0].data?.let {
                    detailListener?.onSuccess(null, it, null, null)
                    return@main
                }
    ...
    fun updateLimit(cin: String, limitAmount: String, userId: Int) {
        detailListener?.onStarted()

        Coroutines.main {
            try {
                val updateLimitResponse = repository.updateLimitParty(cin, limitAmount, userId, "HR")

                updateLimitResponse[0].message?.let {
                    detailListener?.onSuccess(it, null, null, null)
                    return@main
                }
                detailListener?.onFailure("Error","server")
        ...
        Coroutines.main {
            try {
                ...
                getPartyDetailResponse[0].data?.let {
                    detailListener?.onSuccess(null, null, null, it)
                    return@main
                }
                detailListener?.onFailure("Error","server")
